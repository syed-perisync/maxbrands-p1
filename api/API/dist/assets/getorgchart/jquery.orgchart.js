...
(function (factory) {
  ...
  var OrgChart = function OrgChart(elem, opts) {
  ...
  OrgChart.prototype = {
    ...
    setOptions: function setOptions(opts, val) {
    ...
    //
    panEndHandler: function panEndHandler(e) {
    ...
    //
    zoomWheelHandler: function zoomWheelHandler(e) {
      var oc = e.data.oc;
      e.preventDefault();
    ...
    //
    zoomStartHandler: function zoomStartHandler(e) {
    ...
    zoomingHandler: function zoomingHandler(e) {
      var oc = e.data.oc;
    ...
    zoomEndHandler: function zoomEndHandler(e) {
      var oc = e.data.oc;
      if (oc.$chart.data('pinching')) {
    ...
    //
    getPinchDist: function getPinchDist(e) {
    ...
    //
    setChartScale: function setChartScale($chart, newScale) {
    ...
    //
    buildJsonDS: function buildJsonDS($li) {
      var that = this;
      var subObj = {
    ...
    //
    attachRel: function attachRel(data, flags) {
      var that = this;
    ...
    //
    loopChart: function loopChart($chart) {
      var that = this;
    ...
    getNodeState: function getNodeState($node, relation) {
      var $target = {};
      ...
      if (relation === 'parent') {
        ...
        $target = $node;
        if ($target.length) {
          if (!($target.closest('.nodes').length && $target.closest('.nodes').is('.hidden') || $target.closest('table').parent().length && $target.closest('table').parent().is('.hidden') || $target.parent().is('li') && ($target.closest('ul').is('.hidden') || $target.closest('verticalNodes').is('.hidden')))) {
            return { 'exist': true, 'visible': true };
          }
          return { 'exist': true, 'visible': false };
        }
    ...
    getRelatedNodes: function getRelatedNodes($node, relation) {
    ...
    showParentEnd: function showParentEnd(event) {
    ...
    showParent: function showParent($node) {
    ...
    stopAjax: function stopAjax($nodeLevel) {
      if ($nodeLevel.find('.spinner').length) {
        $nodeLevel.closest('.orgchart').data('inAjax', false);
      }
    },
    isVisibleNode: function isVisibleNode(index, elem) {
      return this.getNodeState($(elem)).visible;
    },
    //
    hideChildrenEnd: function hideChildrenEnd(event) {
      var $node = event.data.node;
    ...
    hideChildren: function hideChildren($node) {
    ...
    //
    showChildrenEnd: function showChildrenEnd(event) {
      var $node = event.data.node;
      event.data.animatedNodes.removeClass('sliding');
      if (this.isInAction($node)) {
        this.switchVerticalArrow($node.children('.bottomEdge'));
    ...
    showChildren: function showChildren($node) {
      var that = this;
      var $levels = $node.closest('tr').siblings();
      var isVerticalDesc = $levels.is('.verticalNodes') ? true : false;
      var $animatedNodes = isVerticalDesc ? $levels.removeClass('hidden').find('.node').filter(this.isVisibleNode.bind(this)) : $levels.removeClass('hidden').eq(2).children().find('.node:first').filter(this.isVisibleNode.bind(this));
      // the two following statements are used to enforce browser to repaint
      this.repaint($animatedNodes.get(0));
      $animatedNodes.addClass('sliding').removeClass('slide-up').eq(0).one('transitionend', { 'node': $node, 'animatedNodes': $animatedNodes }, this.showChildrenEnd.bind(this));
    },
    //
    hideSiblingsEnd: function hideSiblingsEnd(event) {
      var $node = event.data.node;
      ...
      event.data.animatedNodes.removeClass('sliding');
      $siblings.find('.node:gt(0)').filter(this.isVisibleNode.bind(this)).removeClass('slide-left slide-right').addClass('slide-up');
      $siblings.find('.lines, .nodes, .verticalNodes').addClass('hidden').end().addClass('hidden');
    ...
    hideSiblings: function hideSiblings($node, direction) {
      var that = this;
    ...
    //
    showSiblingsEnd: function showSiblingsEnd(event) {
      var $node = event.data.node;
    ...
    //
    showRelatedParentEnd: function showRelatedParentEnd(event) {
      $(event.target).removeClass('sliding');
    },
    // show the sibling nodes of the specified node
    showSiblings: function showSiblings($node, direction) {
      var that = this;
    ...
    startLoading: function startLoading($edge) {
      ...
      $edge.parent().append('<i class="fa fa-circle-o-notch fa-spin spinner"></i>').children().not('.spinner').css('opacity', 0.2);
      $chart.data('inAjax', true);
      $('.oc-export-btn' + (this.options.chartClass !== '' ? '.' + this.options.chartClass : '')).prop('disabled', true);
      return true;
    },
    // terminate loading status for requesting new nodes
    endLoading: function endLoading($edge) {
    ...
    isInAction: function isInAction($node) {
      return $node.children('.edge').attr('class').indexOf('fa-') > -1 ? true : false;
    },
    //
    switchVerticalArrow: function switchVerticalArrow($arrow) {
      $arrow.toggleClass('fa-chevron-up').toggleClass('fa-chevron-down');
    },
    //
    switchHorizontalArrow: function switchHorizontalArrow($node) {
    ...
    //
    repaint: function repaint(node) {
      if (node) {
        node.style.offsetWidth = node.offsetWidth;
      }
    },
    //
    nodeEnterLeaveHandler: function nodeEnterLeaveHandler(event) {
    ...
    //
    nodeClickHandler: function nodeClickHandler(event) {
      this.$chart.find('.focused').removeClass('focused');
      $(event.delegateTarget).addClass('focused');
    },
    // load new nodes by ajax
    loadNodes: function loadNodes(rel, url, $edge) {
      var that = this;
      var opts = this.options;
      $.ajax({ 'url': url, 'dataType': 'json', headers: { "x-auth-token": localStorage.getItem("x-auth-token") } }).done(function (data) {
    ...
    //
    HideFirstParentEnd: function HideFirstParentEnd(event) {
    ...
    //
    topEdgeClickHandler: function topEdgeClickHandler(event) {
    ...
    //
    bottomEdgeClickHandler: function bottomEdgeClickHandler(event) {
      event.stopPropagation();
    ...
    //
    hEdgeClickHandler: function hEdgeClickHandler(event) {
      event.stopPropagation();
      ...
      if (siblingsState.exist) {
        ...
        if (this.startLoading($hEdge)) {
          ...
          var url = this.getNodeState($node, 'parent').exist ? $.isFunction(opts.ajaxURL.siblings) ? opts.ajaxURL.siblings($node.data('nodeData')) : opts.ajaxURL.siblings + nodeId : $.isFunction(opts.ajaxURL.families) ? opts.ajaxURL.families($node.data('nodeData')) : opts.ajaxURL.families + nodeId;
          this.loadNodes('siblings', url, $hEdge);
        }
      }
    },
    //
    expandVNodesEnd: function expandVNodesEnd(event) {
      event.data.vNodes.removeClass('sliding');
    },
    //
    collapseVNodesEnd: function collapseVNodesEnd(event) {
      event.data.vNodes.removeClass('sliding').closest('ul').addClass('hidden');
    },
    // event handler for toggle buttons in Hybrid(horizontal + vertical) OrgChart
    toggleVNodes: function toggleVNodes(event) {
    ...
    //
    createGhostNode: function createGhostNode(event) {
    ...
    //
    filterAllowedDropNodes: function filterAllowedDropNodes($dragged) {
      var opts = this.options;
      var $dragZone = $dragged.closest('.nodes').siblings().eq(0).find('.node:first');
      var $dragHier = $dragged.closest('table').find('.node');
      this.$chart.data('dragged', $dragged).find('.node').each(function (index, node) {
    ...
    //
    dragoverHandler: function dragoverHandler(event) {
    ...
    simulateMouseEvent: function simulateMouseEvent(event, simulatedType) {
    ...
    //
    bindDragDrop: function bindDragDrop($node) {
    ...
    createNode: function createNode(data) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>').addClass('node ' + (data.className || '') + (level > opts.visibleLevel ? ' slide-up' : ''));
      if (opts.nodeTemplate) {
        $nodeDiv.append(opts.nodeTemplate(data));
      } else {
        $nodeDiv.append('<div class="title">' + data[opts.nodeTitle] + '</div>').append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
      }
      //
    ...
    buildHierarchy: function buildHierarchy($appendTo, data) {
      var that = this;
      var opts = this.options;
      var level = 0;
      if (data.level) {
      ...
      if (hasChildren) {
        ...
        if (isVerticalLayer) {
          ...
          if (level + 1 === opts.verticalLevel) {
            $appendTo.children('table').append('<tr class="verticalNodes' + isHidden + '"><td></td></tr>').find('.verticalNodes').children().append($nodesLayer);
          } else {
    ...
    buildChildNode: function buildChildNode($appendTo, data) {
      $appendTo.find('td:first').attr('colspan', data.length * 2);
      this.buildHierarchy($appendTo, { 'children': data });
    },
    // exposed method
    addChildren: function addChildren($node, data) {
    ...
    buildParentNode: function buildParentNode($currentRoot, data) {
      data.relationship = data.relationship || '001';
      var $table = $('<table>').append($('<tr>').append($('<td colspan="2">').append(this.createNode(data)))).append('<tr class="lines"><td colspan="2"><div class="downLine"></div></td></tr>').append('<tr class="lines"><td class="rightLine"></td><td class="leftLine"></td></tr>');
      this.$chart.prepend($table).children('table:first').append('<tr class="nodes"><td colspan="2"></td></tr>').children('tr:last').children().append(this.$chart.children('table').last());
    },
    // exposed method
    addParent: function addParent($currentRoot, data) {
    ...
    complementLine: function complementLine($oneSibling, siblingCount, existingSibligCount) {
      var lines = '';
    ...
    buildSiblingNode: function buildSiblingNode($nodeChart, data) {
      ...
      if ($nodeChart.parent().is('td')) {
        ...
        this.complementLine($nodeChart.next().children('tr:last').children().eq(insertPostion).after($('<td colspan="2">').append($nodeChart)), siblingCount, 1);
      }
    },
    //
    addSiblings: function addSiblings($node, data) {
    ...
    //
    removeNodes: function removeNodes($node) {
    ...
    //
    export: function _export(exportFilename, exportFileextension) {
      var that = this;
      ...
      html2canvas(sourceChart, {
        ...
        'onclone': function onclone(cloneDoc) {
          $(cloneDoc).find('.canvasContainer').css('overflow', 'visible').find('.orgchart:not(".hidden"):first').css('transform', '');
        },
          ...
          if (exportFileextension.toLowerCase() === 'pdf') {
            ...
            if (!isWebkit && !isFf || isEdge) {
              ...
              if (!$chartContainer.find(selector).length) {
                $chartContainer.append('<a class="oc-download-btn' + (that.options.chartClass !== '' ? ' ' + that.options.chartClass : '') + '"' + ' download="' + exportFilename + '.png"></a>');
              }
